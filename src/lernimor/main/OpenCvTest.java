...
public class OpenCvTest implements Contants{
    public static String path = "C:/Users/junheng.lv/Documents/lernimor/myPro2/Tess4jTools/src/lernimor/main/";

    static {
        System.load(System.getProperty("user.dir") +
            "/src/x64/opencv_java411.dll");
    }
    ...
    private static void cutTable() {
        Mat src = Imgcodecs.imread(path + "test.jpg");
        ...
        */
        ...
        Imgproc.adaptiveThreshold(xx, thresh, 255,
            Imgproc.ADAPTIVE_THRESH_MEAN_C, Imgproc.THRESH_BINARY, 15, -2);
        ...
        */
        ...
        for (int i = 0; i < contours.size(); i++) {
            ...
            */
            Imgproc.approxPolyDP(new MatOfPoint2f(point.toArray()),
                new MatOfPoint2f(contours_poly_point.toArray()), 3, true);
            ...
            Imgproc.findContours(table_image, table_contours, joint_mat,
        ...
        for (int i = 0; i < tables.size(); i++) {
            ...
            String px_biao = "5";
            int x_len = 0;
            int x_biao = Integer.parseInt(px_biao);
            ...
            for (int j = 0; j < x_z.length; j++) {
                if (j == 0) {
                    ...
                    Mat img = new Mat(table,
                            new Rect(x_z[j - 1], 0, x_z[j] - x_z[j - 1], height));
                    ...
                    if (j == (x_z.length - 1)) { //最后一个处理
                        ...
                        Mat img1 = new Mat(table,
                                new Rect(x_z[x_z.length - 1], 0,
                                    width - x_z[x_z.length - 1], height));
            ...
            imshow(path+"img/", table, "table_" + i + ".png"); //当前table图
                                                            //横切保存

            String py_biao = "5";
            int y_len = 0;
            int y_biao = Integer.parseInt(py_biao);
            
            String[][] matTable = new String[100][mats.size()];

            for (int j = 0; j < mats.size(); j++) {
                Mat mat = mats.get(j);
                int tuwidth = mat.cols();
                int tugao = mat.rows();
                int cy_len = 0;
                
                for (int k = 0; k < y_z.length; k++) {
                    if (k == 0) {
                        Mat img = new Mat(mat, new Rect(1, 1, 
                        		tuwidth - 2 >=0 ? tuwidth - 2 : 0, 
                        		y_z[k] - 2 >=0 ? y_z[k] - 2 : 0));

                        if (img.rows() > y_biao) {
                        	matTable[cy_len][j] = Contants.IMG_NAME + i + "_" + j + "_" + cy_len + Contants.IMG_TYPE;
                            imshow(path+"img/", img, matTable[cy_len][j]);
                            cy_len++;
                        }
                    } else {
                        Mat img = new Mat(mat,
                                new Rect(1, 
                                		y_z[k - 1] + 1, 
                                		tuwidth - 2 >=0 ? tuwidth - 2 : 0, 
                                		y_z[k] - y_z[k - 1] - 2 >= 0 ? y_z[k] - y_z[k - 1] - 2 : 0));
                        if (img.rows() > y_biao) {
                        	matTable[cy_len][j] = Contants.IMG_NAME + i + "_" + j + "_" + cy_len + Contants.IMG_TYPE;
                            imshow(path+"img/", img, matTable[cy_len][j]);
                            cy_len++;
                        ...
                        if (k == (y_z.length - 1)) { //最后一个处理
                            ...
                            Mat img1 = new Mat(mat,
                                    new Rect(1, 
                                    		y_z[k] + 1, 
                                    		tuwidth - 2 >=0 ? tuwidth - 2 : 0, 
                                    		tugao - y_z[k] - 2 >= 0 ? tugao - y_z[k] - 2 : 0));

                            if (img.rows() > y_biao) {
                            	matTable[cy_len][j] = Contants.IMG_NAME + i + "_" + j + "_" + cy_len + Contants.IMG_TYPE;
                                imshow(path+"img/", img1, matTable[cy_len][j]);
                            }
                        }
                    }
                }
            ...
            OcrAction.getCellValueByOpenCv(matTable);
            //保存数据信息
